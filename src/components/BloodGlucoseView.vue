<template>
  <div class="app-container">
          ...
          <form class="form-modal" @submit.prevent="dialogOk">
            ...
            <div class="form-item">
              ...
              <div class="number-input-group">
                <button type="button" class="number-btn" @click="bloodGlucose.bgl = Math.max(3, bloodGlucose.bgl - 0.1)">-</button>
                <input 
                  type="number" 
                  class="form-input number-input" 
                  v-model="bloodGlucose.bgl" 
                  placeholder="正常范围：3.9-6.1mmol/L"
                  min="3"
                  max="20"
                  step="0.1"
                  required
                >
                <button type="button" class="number-btn" @click="bloodGlucose.bgl = Math.min(20, bloodGlucose.bgl + 0.1)">+</button>
              </div>
            </div>
          </form>
        </div>
    ...
    <div class="search-card">
      <form class="search-form" @submit.prevent="handleSearch">
        <div class="search-row">
          ...
          <div class="search-item">
            <label class="search-label">血糖最小值</label>
            <input 
              type="number" 
              class="search-input" 
              v-model="searchForm.minBgl" 
              placeholder="≥ 血糖值"
              @keyup.enter="handleSearch"
              min="3"
              max="20"
          ...
          <div class="search-item">
            <label class="search-label">血糖最大值</label>
            <input 
              type="number" 
              class="search-input" 
              v-model="searchForm.maxBgl" 
              placeholder="＜ 血糖值"
              @keyup.enter="handleSearch"
              min="3"
              max="20"
    ...
    <div class="table-container">
      ...
      <!-- 空数据状态 -->
      <div class="empty-state" v-else-if="bloodGlucoses.length === 0 && !tableLoading">
      ...
      <table class="data-table" v-else>
        ...
        <tbody>
          <tr class="table-tr" :class="rowIndex % 2 === 0 ? 'tr-even' : 'tr-odd'" v-for="(item, rowIndex) in bloodGlucoses" :key="item.id">
            <td class="table-td">{{ item.name }}</td>
            <td class="table-td">{{ item.icn }}</td>
            <td class="table-td">
              <span class="glucose-tag" :class="getGlucoseClass(item.bgl)">
                {{ item.bgl.toFixed(1) }} mmol/L
              </span>
            </td>
...
export default {
  ...
  setup() {
    ...
    const dialogTitle = ref("添加血糖记录");
    const bloodGlucose = reactive({
      name: "",
      icn: "",
      bgl: 5.0  // 默认正常血糖值
    });
    const bloodGlucoses = ref([]);
    ...
    // 方法定义
    // 表格行样式：区分正常/低血糖/高血糖
    const getGlucoseClass = (bgl) => {
      if (bgl < 3.9) return "glucose-low";
      if (bgl <= 7) return "glucose-normal";
      return "glucose-high";
    };
    ...
    const handleAdd = () => {
      ...
      bloodGlucose.name = "";
      bloodGlucose.icn = "";
      bloodGlucose.bgl = 5.0;
      dialogTitle.value = "添加血糖记录";
    ...
    const handleEdit = (index, item) => {
      // 深拷贝数据，避免直接修改原数据
      bloodGlucose.name = item.name;
      bloodGlucose.icn = item.icn;
      bloodGlucose.bgl = item.bgl;
      bloodGlucose.id = item.id;  // 携带ID用于编辑
      dialogTitle.value = "编辑血糖记录";
      dialogVisible.value = true;
    };
    ...
    const dialogOk = () => {
      ...
      axios.get(url, { params: bloodGlucose })
        .then(response => {
    ...
    const handleSearch = (act) => {
      ...
      searchParams.minBgl = searchForm.minBgl || null;
      searchParams.maxBgl = searchForm.maxBgl || null;
      // 转换时间格式（适配后端需求）
      ...
      // 1. 先获取总条数，计算总页数
      axios.get('/bloodglucose/count', { params: searchParams })
        .then(res => {
          if (res.data.code === 0) {
            ...
            // 2. 获取当前页数据
            return axios.get('/bloodglucose/find', { params: searchParams });
          } else {
        ...
        .then(res => {
          if (res.data.code === 0) {
            bloodGlucoses.value = res.data.data || [];
          } else {
            bloodGlucoses.value = [];
    ...
    const handleDelete = (index, item) => {
      if (confirm(`此操作将永久删除${item.name}的血糖记录，是否继续？`)) {
        tableLoading.value = true;
        axios.get('/bloodglucose/del', { params: { id: item.id } })
          .then(response => {
            if (response.status === 200 && response.data.code === 0) {
              showToast(`成功删除${item.name}的血糖记录`, "success");
              handleSearch();  // 重新加载数据
            } else {
